...
class Player(Creature):
    ...
    def update(self):
        ...
        for p in game.mplatforms:
            if self.isAbove(p) and not self.keyHandler['UP']: #(if character is above the platform AND within the width of platform) and not self.isInside(p)
        ...
        if self.keyHandler['UP'] and self.g == self.y+self.r: #and self.grav==0: #AND statement ensures that we cannot double jump
            self.vy = -10
...
class Game:
    ...
    def lvl1(self):
        ...
        self.buttons.append(Button(0,540,50,10)) #BUTTON FOR BARRIER
                
        #self.platforms.append(Platform(0,718,1440,50)) #Ground
        
        self.door=Door(400,50,50,80) #Door
        self.gate=Gate(600,250,50,80) #Gate
    ...
    def lvl2(self):
        ...
        self.platforms.append(Platform(0,150,1216,50)) #Platform-3
        #self.platforms.append(Platform(400,350,1100,50)) #Platform-2
        self.mplatforms.append(mPlatform(self.w-200,651,200,50,1,200,650)) #Platform-1 MOVING
...
game = Game(1440,768,718,2) #Window dimensions and the ground value
